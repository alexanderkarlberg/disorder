...
\NNNLO{} with integration uncertainties typically below the permille
level. For exclusive quantities the code can be run on a laptop at NLO
and depending on the analysis, and the laptop, even at NNLO.
...
To compile \disorder{} both \hoppet{} and {\tt
...
the Breit and laboratory frame momenta are stored respectively. The
output of the analysis will be saved to the disk as outlined below.
...
that the actual \NNNLO{} results will be contained within the scale
variation band at NNLO.
...
be efficient. The upgrade of the code to handle \NNNLO{} fully
...
a proper exclusive NNLO DIS + 1 jet code, requiring a significant
...
even reasonable NNLO results can be obtained on a single machine
running for a few hours. \NNNLO{} results will inevitably require
...
Gavin Salam regarding {\tt disent}, and for encouragement to publish
\disorder{}. The idea for the code originated while the author was
working on Ref.~\cite{Banfi:2023mhz}.
