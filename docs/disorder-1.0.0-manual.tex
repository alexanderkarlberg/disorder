...
very little effort. It comes with an interface to {\tt
  fastjet}~\cite{Cacciari:2011ma} and {\tt
  LHAPDF}~\cite{Buckley:2014ana} and uses the histogramming package
from the {\tt POWHEG-BOX}~\cite{Alioli:2010xd} for easy analysis. The
code can also compute renormalisation and factorisation scale
uncertainties on-the-fly. For inclusive cross sections the code
provides results in a matter of seconds\footnote{The majority of this
time is taken up by the structure function initialisation inside
\hoppet{} rather than the integration of the cross section.} even at
\NNNLO{} with uncertainties typically below the permille level. For
exclusive quantities the code can be run on a laptop at NLO and
depending on the analysis, and the laptop, even at NNLO.
...
vector $q$ is hence given by $Q(0,0,0,-1)$. In the laboratory frame we align
...
In principle P2B can be applied to the \disent{} code without any
...
the Breit and laboratory frame momenta are stored respectively. The output of
...
photon-mediated NC cross section only. To include the $Z$ one can specify
...
``{\tt{no}}'' to turn them off. Hence the below command line would run the
...
scales if {\tt -scaleuncert} is on.

The user can specify a prefix to be added to all the files through the
{\tt -prefix} flag.
...
better than permille level accuracy. If the user wishes to for
instance bin the cross section in fine bins of $x$ and $Q^2$, this
number will most likely have to increase.
...
\begin{table}[t] 
  \centering
  \phantom{x}\medskip% add space to make table look nicer...
...
full \NNNLO{} to assess the reliability of the NNLO result). 
...
extremely fast, and a user can obtain cross section results in a
matter of seconds.

In the fully differential mode, only photon-mediated DIS can be
...
this would require some serious restructuring of the code, for it to
be efficient. At LO and NLO a user can obtain differential results on
a laptop running for a just a few minutes, and even reasonable NNLO
results can be obtained on a single machine running for a few hours.

The code comes with an interface to {\tt fastjet} and {\tt LHAPDF} and
...
The author is grateful for discussions with Gavin Salam regarding {\tt
  disent}, and for encouragement to publish \disorder{} from Andrea
Banfi. The idea for the code originated while the author was working
on Ref.~\cite{Banfi:2023mhz}.
...
% # QCD scale uncertainty (+)      =        0.5466 %
% # QCD scale uncertainty (-)      =       -0.1592 %
% # PDF symmetric uncertainty*     =        0.8460 %
