! Adapted from dummy_pdfs in Hoppet to include the toy PDF initial
! condition used in PanScales
module toy_pdfs
  ...
  public :: unpolarized_toy_nf5_pdf, lha_unpolarized_toy_nf5_pdf
...
contains
  ...
  !! The dummy PDF suggested by Vogt as the initial condition for the 
  !! unpolazrized evolution
  pure subroutine lha_unpolarized_toy_nf5_pdf(x, Q, xpdf)
    real(dp), intent(in)  :: x,Q
    real(dp), intent(out) :: xpdf(-6:6)
    real(dp) :: uv, dv
    real(dp) :: ubar, dbar
    ...
    xpdf(-iflv_s) = 0.2_dp*(dbar + ubar)
    xpdf(iflv_s) = xpdf(-iflv_s)
    xpdf(iflv_u) = uv + 0.8_dp * ubar
    xpdf(-iflv_u) = 0.8_dp * ubar
    xpdf(iflv_d) = dv + 0.8_dp * dbar
    xpdf(-iflv_d) = 0.8_dp * dbar

    ! Some charm and bottom PDF here
    xpdf(iflv_c) = 0.15_dp * (ubar + dbar)
    xpdf(-iflv_c) = xpdf(iflv_c) 
    xpdf(iflv_b) = 0.05_dp * (ubar + dbar)
    xpdf(-iflv_b) = xpdf(iflv_b) 
end subroutine lha_unpolarized_toy_nf5_pdf
  

  !----------------------------------------------------------------------
  ! the same dummy PDF, in a slightly different interface
  function unpolarized_toy_nf5_pdf(xvals) result(pdf)
    real(dp), intent(in) :: xvals(:)
    real(dp)             :: pdf(size(xvals),ncompmin:ncompmax)
    !---------------------
    integer  :: iy
    ...
    do iy = 1, size(xvals)
       call lha_unpolarized_toy_nf5_pdf(xvals(iy), dummy_Q, pdf(iy, iflv_min:iflv_max))
    end do
  end function unpolarized_toy_nf5_pdf
