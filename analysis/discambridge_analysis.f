...
!     Back to 1:4 index
      ...
      subroutine buildjetscentauro(n, pin, R, pj, njets)
      implicit none
      integer n
      double precision pin(0:3,n), R
      integer maxtrack,maxjet
      parameter (maxtrack=3,maxjet=3)
...
!     Fast jet needs 0 indexed tracks
      ...
      call fastjetcentauro(ptrack,ntracks,R,pjet,njets)
...
!     Back to 1:4 index
      ...
      integer function macro_jet_index(pj,njets,beam_sign,ordering)
      implicit none
      integer maxjets
      parameter (maxjets = 3)
      double precision pj(0:3,maxjets)
      integer njets, beam_sign
      integer ordering ! 0: longitudinal ordering, 1: rapidity ordering
      integer i, imacro
      double precision longmax, long, yj, eta, getrapidity0
      longmax = -1d100
      do i=1,njets
         if(ordering.eq.0) then
            long = pj(0,i) - pj(3,i)*beam_sign
            if (long.gt.longmax) then
               longmax = long
               imacro = i
            endif
         else if(ordering.eq.1) then
            yj = -getrapidity0(pj(:,i)) * beam_sign
            if (yj.gt.longmax) then
               longmax = yj
               imacro = i
            endif
         endif
